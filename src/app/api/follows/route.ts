...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) return NextResponse.json({ message: 'Fetch failed', details: error.message }, { status: 400 });
    return NextResponse.json(data ?? [], { status: 200 });
  } catch (err: any) {
    if (err.message === 'Rate limit exceeded') {
      return NextResponse.json({ message: 'Rate limit exceeded' }, { status: 429 });
    }
    return NextResponse.json({ message: 'Fetch follows failed', details: err?.message || String(err) }, { status: 500 });
  }
}
